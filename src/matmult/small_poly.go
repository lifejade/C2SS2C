...
func (pol *Poly) Resize(level int) {
	...
	if pol.Level() > level {
		...
		prevLevel := pol.Level()
		pol.Coeffs = append(pol.Coeffs, make([][]uint32, level-prevLevel)...)
		for i := prevLevel + 1; i < level+1; i++ {
			pol.Coeffs[i] = make([]uint32, N)
		}
	}
}
...
func ZeroVec(p1 []uint32) {
	...
	for j := 0; j < N; j = j + 8 {
		...
		z := (*[8]uint32)(unsafe.Pointer(&p1[j]))
