...
func (be *BasisExtender) ModUpPtoQ2(levelP, levelQ int, polP, polQ ring.Poly) {
...
func ModUpExact2(p1, p2 [][]uint64, ringQ, ringP *ring.Ring, MUC ModUpConstants) {
...
func (decomposer *Decomposer) DecomposeAndSplit(levelQ, levelP, nbPi, BaseRNSDecompositionVectorSize int, p0Q, p1Q, p1P ring.Poly) {
...
func reconstructRNSCentered(start, end, x int, p [][]uint64, v *[8]uint64, vi *[8]float64, y0, y1, y2, y3, y4, y5, y6, y7 *[32]uint64, QHalfModqi, Q, mredQ, qoverqiinvqi []uint64) {
	...
	for i, j := 0, start; j < end; i, j = i+1, j+1 {
		...
		y0[i] = ring.MRed(px[0]+qHalf, qqiinv, qi, mredConstant)
		y1[i] = ring.MRed(px[1]+qHalf, qqiinv, qi, mredConstant)
		y2[i] = ring.MRed(px[2]+qHalf, qqiinv, qi, mredConstant)
		y3[i] = ring.MRed(px[3]+qHalf, qqiinv, qi, mredConstant)
		y4[i] = ring.MRed(px[4]+qHalf, qqiinv, qi, mredConstant)
		y5[i] = ring.MRed(px[5]+qHalf, qqiinv, qi, mredConstant)
		y6[i] = ring.MRed(px[6]+qHalf, qqiinv, qi, mredConstant)
		y7[i] = ring.MRed(px[7]+qHalf, qqiinv, qi, mredConstant)
