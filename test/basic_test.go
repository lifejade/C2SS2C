...
// 	fmt.Println("ckks parameter init end")
// 	Q := params.Q()
// 	P := []uint32{4107427, 3868699, 4073143, 3639397, 3835109, 3377447, 3338903, 3314141, 3816173, 3731251, 3925091, 3500261, 3507403, 3368353, 3598601, 3637573, 3387523, 3489259, 3804751, 4002811, 3417251, 3245357, 3659177, 4047647, 3367981, 3984439, 3621473, 3565147, 3789193, 3174547, 3293959, 3567803, 3856499, 3299617, 3939619, 4004683, 3803347, 3501467, 3518719, 3631919}
// 	PLevel := 35
// 	ringQ, _ := ring.NewRing(params.N(), Q)
// 	ringP, _ := matmult.NewRing(params.N(), P[:PLevel+1])
...
// 				for k := range u[i][j] {
// 					u[i][j][k] = uint64(0.5*scale) % uint64(ringP.ModuliChain()[i])
// 				}
// 			}
// 		}

// 		rings := make([][]matmult.Poly, 2)
// 		for idx := range 2 {
// 			rings[idx] = make([]matmult.Poly, sizes[step])
...
// 	sparseSize := []int{12, 13, 14, 15, 16}
// 	for _, s := range sparseSize {
// 		fmt.Println("//////////////////////////////////////////////////////////////")
// 		var totaltime time.Duration
// 		sparseN := 1 << s
// 		starttime = time.Now()
...
// 			if step == 0 {
// 				fmt.Println("steptime : ", steptime)
// 				steptime = steptime * 8 * time.Duration(sizes[1])
// 				fmt.Println("steptotaltime : ", math.Round(steptime.Seconds()*100)/100)
// 			} else {
// 				fmt.Println("steptime : ", steptime)
// 				steptime = steptime * 8 * time.Duration(sizes[0])
// 				fmt.Println("steptotaltime : ", math.Round(steptime.Seconds()*100)/100)
// 			}
// 			totaltime += steptime
// 		}
...
// 		util.Rescale_NonNTT(evaluator, ct, ct)
// 		elapse_ = time.Since(time_)
// 		fmt.Println("rescale", math.Round((elapse_*(time.Duration(sparseN))).Seconds()*100)/100)
// 		totaltime += elapse_ * (time.Duration(sparseN))
// 		fmt.Println("totaltime (cal.) : ", math.Round(totaltime.Seconds()*100)/100)
// 		ringQ.AtLevel(ct.Level()).NTT(ct.Value[0], ct.Value[0])
// 		ringQ.AtLevel(ct.Level()).NTT(ct.Value[1], ct.Value[1])
...
// func Test_CheckTimePCMMSparse_Inplace(t *testing.T) {
// 	runtime.GOMAXPROCS(1) // CPU 개수를 구한 뒤 사용할 최대 CPU 개수 설정
// 	SchemeParams := hefloat.ParametersLiteral{
// 		LogN:            16,
// 		LogQ:            []int{48, 40, 48, 48, 48, 48, 48, 48, 48, 48, 48, 48, 48, 40, 40},
...
// 			size := sizes[step]
// 			index := 0
// 			u := make([]float64, (PLevel+1)*size*size)
// 			for i := range PLevel + 1 {
// 				for range size {
// 					for range size {
// 						u[index] = float64(uint64(0.5*scale) % ringP.ModuliChain()[i])
// 						index++
// 					}
// 				}
// 			}
...
// 			buffer1 := make([]float64, size*params.N()*(PLevel+1))
// 			buffer2 := make([]float64, size*params.N()*(PLevel+1))
// 			time_ := time.Now()
// 			matmult.PPMM_Blas_CRT_Inplace(rings, u, size, size, params.N(), PLevel+1, 2, ringP, buffer1, buffer2)
// 			elapse_ := time.Since(time_)
// 			fmt.Println("ppmm", elapse_)
// 			steptime += elapse_
// 			for idx := range 2 {
...
func Test_Ringadd(t *testing.T) {
	...
	ringQ.Add(ct.Value[0], ct2.Value[0], ct.Value[0])
	ringQ.Add(ct.Value[1], ct2.Value[1], ct.Value[1])

	ringQ.NTT(ct.Value[0], ct.Value[0])
	ringQ.NTT(ct.Value[1], ct.Value[1])
	ringQ.NTT(ct2.Value[0], ct2.Value[0])
	ringQ.NTT(ct2.Value[1], ct2.Value[1])
